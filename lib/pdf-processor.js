...
export class PDFProcessor {
  ...
  async loadPDFJS() {
    ...
    // PDF.js should be loaded as a bundled library in the extension
    if (typeof pdfjsLib !== 'undefined') {
      this.pdfjsLib = pdfjsLib;
    } else {
      // Fallback: try importing from bundled location
      const script = await import('../vendor/pdf.min.js');
      this.pdfjsLib = script;
    }

    // Set worker source
    if (this.pdfjsLib.GlobalWorkerOptions) {
      this.pdfjsLib.GlobalWorkerOptions.workerSrc = '../vendor/pdf.worker.min.js';
    }
  }
