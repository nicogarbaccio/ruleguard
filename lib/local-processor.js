...
export class LocalComplianceProcessor {
    ...
    if (!text) {
      try {
        ...
        return [{
          ...
          description: `Local OCR could not run: ${this.describeError(error)}`,
          gliReference: 'N/A',
  ...
  /**
   * Coerce any thrown value (Error, ErrorEvent, string, event) into a useful
   * message so failures never display as "undefined".
   */
  describeError(error) {
    if (!error) return 'unknown error (no details provided)';
    if (typeof error === 'string') return error;
    if (error.message) return error.message;
    if (error.error && error.error.message) return error.error.message;
    if (error.type) return `worker error event: ${error.type}`;
    try {
  ...
  async getOCRWorker() {
    ...
    this.ocrWorker = await Tesseract.createWorker('eng', 1, {
      ...
      // Point directly at the SIMD LSTM core file (not a directory). This
      // avoids the worker's internal directory/SIMD-detection logic, which is
      // unreliable under the MV3 extension CSP.
      ...
      // MV3 extension CSP (script-src 'self') blocks workers created from
      // blob: URLs, which is Tesseract's default. Load the worker directly
      // from the bundled extension URL instead.
      workerBlobURL: false,
      gzip: true,
      cacheMethod: 'none'
    });
