...
Return ONLY valid JSON in this format:
{
    ...
    for (let attempt = 0; attempt < this.maxRetries; attempt++) {
      try {
        ...
        if (response.status === 429) {
          // Rate limited. Read the body once so we can both honor any
          // provider-supplied retry delay AND surface the real reason
          // (e.g. "model has no free tier quota", which quota was exceeded).
          const bodyText = await this.safeReadBody(response);
          const waitTime = this.getRateLimitWaitFromBody(response, bodyText, attempt);
          const detail = this.extractErrorDetail(bodyText);

          const base = `Rate limited by the AI provider (HTTP 429).`;
          const geminiNote = this.provider === 'gemini'
            ? ' The Gemini free tier has tight per-minute/day quotas, and some models (e.g. 2.5 Pro) have no free quota at all — enabling billing on the key\'s Google Cloud project, or switching to a Flash model, usually fixes this.'
            : '';
          lastError = new Error(`${base}${detail ? ' ' + detail : ''}${geminiNote}`);
          lastError.status = 429;

          // Hard quota exhaustion (no free quota / per-day limit) won't clear
          // on a short retry — fail fast instead of burning the retry budget.
          if (this.isHardQuotaExhaustion(bodyText)) {
            throw lastError;
          }
  ...
  /**
   * Detect a 429 that won't clear on a short retry: a per-day quota or a
   * "limit: 0" metric (model has no free quota). Retrying these just wastes
   * the retry budget and delays the inevitable error.
   */
  isHardQuotaExhaustion(bodyText) {
    if (!bodyText) return false;
    if (/"?limit"?\s*:?\s*0\b/.test(bodyText)) return true;
    if (/PerDay|per_day|free_tier/i.test(bodyText)) return true;
    return false;
  }
  ...
  /**
   * Pull a human-readable message out of an error body (OpenAI/Anthropic/
   * Gemini all nest it under error.message).
   */
  extractErrorDetail(bodyText) {
    if (!bodyText) return '';
    try {
      const json = JSON.parse(bodyText);
      return (json?.error?.message || json?.message || '').trim();
    } catch {
  ...
  /**
   * Determine how long to wait before retrying a 429 from an already-read
   * body. Checks the standard Retry-After header, then Gemini's RetryInfo
   * (e.g. "retryDelay": "37s"), then an exponential backoff with a sensible
   * minimum for per-minute quota windows.
   */
  getRateLimitWaitFromBody(response, bodyText, attempt) {
  ...
  async getRateLimitWait(response, attempt) {
    const bodyText = await this.safeReadBody(response);
    return this.getRateLimitWaitFromBody(response, bodyText, attempt);
  }
