...
export class Storage {
  ...
  /**
   * Append captured frames for a game without removing existing ones.
   * Used by the standalone capture window so separate capture sessions
   * accumulate rather than overwrite each other.
   * @param {string} gameId - Game ID
   * @param {Array} frames - Array of { dataUrl, timestamp, width, height }
   * @returns {number} The new total frame count for the game
   */
  async appendFrames(gameId, frames) {
    await this.open();
    ...
    const all = await this.getFrames(gameId);
    await this.updateGame(gameId, { frameCount: all.length });
    return all.length;
  }
